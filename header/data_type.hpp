...
class data_type {
	...
	public:
		data_type () = default;
		data_type (std::size_t rows, std::size_t cols) : rows_{rows}, cols_{cols}, data_ (rows*cols) {}

		int& operator () (std::size_t i, std::size_t j) { return data_.at(rows_*j + i); }
		const int& operator () (std::size_t i, std::size_t j) const { return data_.at (rows_*j + i); }
