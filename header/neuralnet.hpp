...
class NeuralNet {
  private:
    ...
    float initial_connect_weight = 1.0;
    std::vector<std::vector<std::vector<float>>> layers_connects_; // relations of neurons
    // layers_connects_ is vector of connects
    // connects is vector of weights
  ...
  public:
    ...
    inline std::vector<std::vector<vector<float>>> getConnect () const { return layers_connects_; }
...
void NeuralNet::initialization () {
  ...
  layers_connects_.resize (layers_.size () + 1);
  layers_connects_[0].resize (input_size_, std::vector<float>(layers_[0], initial_connect_weight));
  for (std::size_t i=1; i<layers_.size (); ++i) {
    layers_connects_[i].resize (layers_[i-1], std::vector<float>(layers_[i], initial_connect_weight));
  }
  layers_connects_[layers_.size ()].resize (layers_.back (), std::vector<float>(output_size_, initial_connect_weight));
...
void NeuralNet::view_connect () {
  ...
  for (const auto& connects : layers_connects_) {
    for (const auto& weights : connects) {
...
void NeuralNet::back_propagation () {
  for (std::size_t i=layers_connects_.size ()-1; i<layers_connects_.size (); --i) {
    for (std::size_t j=0; j<layers_connects_[i].size (); ++j) {
      for (std::size_t k=0; k<layers_connects_[i][j].size (); ++k) {
        float old_connect = layers_connects_[i][j][k];
        layers_connects_[i][j][k] += nrand_.generate ();
        float new_acc = this->calculate_accuracy (train_data_);
        if (accuracy_ < new_acc) {
          accuracy_ = new_acc;
          //cout << "UPDATE" << endl;
        } else {
          layers_connects_[i][j][k] = old_connect;
          //cout << "REBASE" << endl;
        }
      }
    }
...
void NeuralNet::simple_optim () {
  auto old_connects = layers_connects_;

  for (auto& connects : layers_connects_) {
    for (auto& weights : connects) {
      for (auto& weight : weights) {
        weight += nrand_.generate ();
      }
    }
  }
...
std::vector<float> NeuralNet::predict (data_type const& input_data) const {
  ...
  new_neurons.resize (layers_connects_[0][0].size (), 0.0);
  for (size_t j=0; j<layers_connects_[0].size (); ++j) {
    for (size_t k=0; k<layers_connects_[0][j].size (); ++k) {
      new_neurons[k] += (input_data.at (j) * layers_connects_[0][j][k]);
    }
  }
  ...
  for (std::size_t i=1; i<layers_connects_.size ()-1; ++i) {
    std::swap (old_neurons, new_neurons);
    new_neurons.resize (layers_connects_[i][0].size (), 0.0);

    for (size_t j=0; j<layers_connects_[i].size (); ++j) {
      for (size_t k=0; k<layers_connects_[i][j].size (); ++k) {
        new_neurons[k] += (old_neurons[j] * layers_connects_[i][j][k]);
      }
    }

    for (size_t k=0; k<layers_connects_[i][0].size (); ++k) {
      new_neurons[k] = Activation::unit (neurons_[i][k], new_neurons[k]);
    }
  }

  std::vector<float> predictions (output_size_, 0.0);
  for (size_t j=0; j<layers_connects_.back ().size (); ++j) {
    for (size_t k=0; k<layers_connects_.back ()[j].size (); ++k) {
      predictions[k] += (new_neurons.at (j) * layers_connects_.back ()[j][k]);
    }
  }
  //Debug::view (predictions);
  for (size_t k=0; k<layers_connects_.back ()[0].size (); ++k) {
